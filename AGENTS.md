...
- **Deep research (NEW)**: `docs/LACQUER_DISC_DIGITAL_TWIN.md` — full scientific investigation + original "LACQUER 360" system proposal (7-layer inverted-discovery architecture: ingredient genome; physics kernel w/ UNIFAC+Vrentas–Duda+WLF+Tg; MD calibration from propellant literature — NC/DBP Tg, plasticizer migration, ~50 °C NC transition; cutting-fingerprint model; FDS2S quality surrogate; BoTorch qNEHVI inverted designer; human-in-loop validation ~20–40 rounds). Key novel ideas: the plasticizer-independent ~50 °C NC chain-flexibility transition (Polymer 1988) as the stylus-temp design point; residual solvent as tuning knob; cutterhead-as-sensor fingerprint; FTIR gate (thread 1734); recipe-manifold generation; transfer from 30k-solvent MD dataset
- **Recipe generator (NEW, pushed `2d4c6e9`)**: `core/recipe_generator.py` — working MVP of LACQUER 360 layers L2–L6 (physics kernel + random search + diversity pick). Pure numpy/scipy, no sklearn dep. CLI: `venv/bin/python -m core.recipe_generator --n 8000 --top 5 --seed N [--store] [--report path]` (--store saves candidates into FormulaStore library as `gen_*`). `INGREDIENTS` table (NC + 5 plasticizers + 12 solvents with density/bp/evap/Hansen/Tg); NC_HANSEN=(16.2,14.1,9.5) per Hansen Handbook 2nd ed. (literature values vary wildly, see PubMed 35830376); hard gates: solids 10–28%, plast/NC 0.15–0.60, Hansen dist <9.0, dry 0.5–72h, T_cut−Tg 5–55K; scoring penalizes dev from classic profile (dry ~8h, Tg_room ~30 °C, crater low). Tests: `tests/test_recipe_generator.py` (plain asserts, `venv/bin/python tests/test_recipe_generator.py`). Gotchas: `recipe_data()` must `float()`-coerce rounds (np.float64 breaks FormulaStore YAML safe_dump); no comments in code per AGENTS rule
