...
- **Deep research (NEW)**: `docs/LACQUER_DISC_DIGITAL_TWIN.md` — full investigation + original system "LACQUER 360": 7-layer inverted-discovery architecture (ingredient genome → physics kernel UNIFAC/Vrentas–Duda/WLF/Tg → MD calibration from propellant literature → cutting-fingerprint model → FDS2S quality surrogate → BoTorch qNEHVI inverted designer + ChemCrow-style LLM agent → human-in-loop validation, ~20–40 rounds to ≥ classic quality). Novel research findings embedded: ~50 °C plasticizer-independent NC transition (Polymer 1988 DMA) as stylus-window design point; residual solvent as tuning knob; cutterhead-as-sensor; FTIR fingerprint gate (KB 1734); recipe-manifold generation; transfer learning from 30k-solvent MD mixture dataset
